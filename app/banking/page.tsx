...
const CATEGORY_COLORS: Record<string, string> = {
  Materials:  'bg-[#EBF0FD] text-[#1A3FAA]',
  Equipment:  'bg-[#FEF6E4] text-[#96670A]',
  Labour:     'bg-[#E8F5EE] text-[#1A6B45]',
  Income:     'bg-[#E8F5EE] text-[#1A6B45]',
  Utilities:  'bg-gray-100 text-[#9B978F]',
  Software:   'bg-gray-100 text-[#9B978F]',
  Tax:        'bg-[#FCEAE8] text-[#B8301A]',
}
    ...
    <AppLayout>
      <div className="sticky top-0 z-10 h-12 bg-white border-b border-[#E5E2DB] px-6 flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <Chevron />
          <span className="text-xs font-semibold text-gray-900">Open Banking</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-[#9B978F] flex items-center gap-1">
            <svg width="10" height="10" viewBox="0 0 24 24" fill="none"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" stroke="#1A6B45" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
            Powered by Open Banking · FCA regulated
          </span>
        </div>
      </div>
      ...
      <div className="p-6">
        <div className="mb-5 flex items-start justify-between">
          <div>
            <h1 className="text-xl font-bold tracking-tight text-gray-900">Open Banking</h1>
            <p className="text-xs text-[#9B978F] mt-1">Connect your bank accounts and automatically match transactions to project costs.</p>
          </div>
          {unmatchedCount > 0 && (
            <div className="bg-[#FEF6E4] border border-[#96670A]/20 rounded-lg px-3 py-2 text-xs text-[#96670A] font-semibold">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-white border border-[#DDD9D0] rounded-[10px] shadow-[0_1px_2px_rgba(0,0,0,.05)] p-4">
            <div className="text-[10px] uppercase tracking-wide text-[#9B978F] font-semibold mb-1">Total Balance</div>
            <div className="text-2xl font-bold text-gray-900">{fmt(totalBalance)}</div>
            <div className="text-[11px] text-[#9B978F] mt-0.5">{BANKS.length} accounts connected</div>
          </div>
          <div className="bg-white border border-[#DDD9D0] rounded-[10px] shadow-[0_1px_2px_rgba(0,0,0,.05)] p-4">
            <div className="text-[10px] uppercase tracking-wide text-[#9B978F] font-semibold mb-1">Money In (Jun)</div>
            <div className="text-2xl font-bold text-[#1A6B45]">{fmt(totalIn)}</div>
            <div className="text-[11px] text-[#9B978F] mt-0.5">Client payments &amp; deposits</div>
          </div>
          <div className="bg-white border border-[#DDD9D0] rounded-[10px] shadow-[0_1px_2px_rgba(0,0,0,.05)] p-4">
            <div className="text-[10px] uppercase tracking-wide text-[#9B978F] font-semibold mb-1">Money Out (Jun)</div>
            <div className="text-2xl font-bold text-[#D4561A]">{fmt(totalOut)}</div>
            <div className="text-[11px] text-[#9B978F] mt-0.5">Costs, labour &amp; overheads</div>
          </div>
        </div>

        {/* Bank account tiles */}
        <h2 className="text-xs font-bold text-gray-900 uppercase tracking-wide mb-3">Connected Accounts</h2>
        <div className="grid grid-cols-3 gap-3 mb-6">
          {BANKS.map(bank => (
            <button
              key={bank.id}
              onClick={() => setSelectedBank(bank.id)}
              className={`relative rounded-[12px] p-4 text-left transition-all border-2 ${selectedBank === bank.id ? 'border-[#D4561A] shadow-md' : 'border-transparent shadow-sm hover:shadow-md'}`}
        ...
        <div className="bg-white border border-[#DDD9D0] rounded-[10px] shadow-[0_1px_2px_rgba(0,0,0,.05)] overflow-hidden">
          {transactions.length === 0 ? (
            <div className="py-12 text-center">
              <p className="text-sm text-gray-400 font-semibold">No transactions for this account</p>
            </div>
          ) : (
            <table className="w-full">
              <thead>
                <tr className="bg-[#F7F6F2]">
                  <th className="px-5 py-2.5 text-[9px] uppercase tracking-wide text-[#9B978F] font-semibold text-left">Date</th>
                  <th className="px-4 py-2.5 text-[9px] uppercase tracking-wide text-[#9B978F] font-semibold text-left">Description</th>
                  <th className="px-4 py-2.5 text-[9px] uppercase tracking-wide text-[#9B978F] font-semibold text-left">Category</th>
                  <th className="px-4 py-2.5 text-[9px] uppercase tracking-wide text-[#9B978F] font-semibold text-right">Amount</th>
                  <th className="px-4 py-2.5 text-[9px] uppercase tracking-wide text-[#9B978F] font-semibold text-center">Match</th>
                </tr>
              </thead>
              <tbody>
                {transactions.map(tx => {
                  ...
                  return (
                    <tr key={tx.id} className={`border-t border-[#F0EDE8] hover:bg-[#F7F6F2] transition-colors ${isMatched ? 'bg-[#F0FAF4]' : ''}`}>
                      <td className="px-5 py-3 text-xs text-[#9B978F] tabular-nums whitespace-nowrap">{tx.date}</td>
                      <td className="px-4 py-3">
                        <div className="text-xs font-semibold text-gray-900">{tx.description}</div>
                        {isMatched && <div className="text-[10px] text-[#1A6B45]">Matched to cost record</div>}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[9px] font-bold ${CATEGORY_COLORS[tx.category] || 'bg-gray-100 text-gray-500'}`}>
                          {tx.category}
                        </span>
                      </td>
                      <td className={`px-4 py-3 text-xs font-mono font-bold text-right tabular-nums ${tx.type === 'credit' ? 'text-[#1A6B45]' : 'text-gray-900'}`}>
                      ...
                      <td className="px-4 py-3 text-center">
                        {tx.type === 'debit' && (
                          <button
                            ...
                            className={`text-[9px] font-bold px-2 py-1 rounded-lg transition-colors ${isMatched ? 'bg-[#E8F5EE] text-[#1A6B45] hover:bg-[#d4f0e0]' : 'bg-[#F0EDE8] text-[#9B978F] hover:bg-[#E5E2DB] hover:text-gray-700'}`}
                          >
                            {isMatched ? 'Matched' : 'Match'}
                          </button>
                        )}
                        {tx.type === 'credit' && (
                          <span className="text-[9px] text-[#9B978F]">Income</span>
                        )}
                      </td>
                    </tr>
                  )
        ...
        {/* FCA badge */}
        <div className="mt-4 flex items-center gap-2 text-[10px] text-[#9B978F]">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" stroke="#9B978F" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowConnect(false)}>
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-[#E5E2DB] flex items-center justify-between">
              <h3 className="font-bold text-sm text-gray-900">Connect {connectingBank.name}</h3>
              <button onClick={() => setShowConnect(false)} className="text-[#9B978F] hover:text-gray-700">
            ...
            <div className="p-6 text-center">
              ...
              <p className="text-sm font-semibold text-gray-900 mb-1">Secure Open Banking connection</p>
              <p className="text-xs text-[#9B978F] mb-4">You&apos;ll be redirected to {connectingBank.name} to authorise read-only access. This is a secure FCA-regulated flow.</p>
              <div className="space-y-2 text-left bg-[#F7F6F2] rounded-lg p-3 mb-4">
              ...
              <button onClick={() => { setShowConnect(false); toast(`${connectingBank.name} connection refreshed`) }} className="w-full bg-[#D4561A] text-white text-xs font-semibold py-2.5 rounded-lg hover:bg-[#BE4A16]">
